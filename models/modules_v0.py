...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, inc=64, outc=64, groups=1, scale=1.0):
        ...
        self.bn1 = nn.GroupNorm(32, midc)
        self.relu1 = nn.LeakyReLU(0.2, inplace=True)
        self.conv2 = nn.Conv2d(in_channels=midc, out_channels=outc, kernel_size=3, stride=1, padding=1, groups=groups,
                               bias=False)
        # self.bn2 = nn.BatchNorm2d(outc)
        self.bn2 = nn.GroupNorm(32, outc)
...
class Encoder(nn.Module):
    def __init__(self, cdim=3, zdim=64, channels=(64, 128, 256), image_size=32, conditional=False,
                 cond_dim=10, **kwargs):
        ...
        if self.conditional:
            self.fc = nn.Linear(num_fc_features + self.cond_dim, zdim)
        else:
...
class Decoder(nn.Module):
    def __init__(self, cdim=3, zdim=64, channels=(64, 128, 256), image_size=32, conditional=False,
                 conv_input_size=(256, 4, 4), cond_dim=10, **kwargs):
        ...
        # self.sigma_type = kwargs.get('sigma_type', 'z')
        # if 'sigma_type' not in kwargs or kwargs['sigma_type'] == 'z':
        #     # Modeled as a Gaussian: mu, logvar.
        #     self.main.add_module('predict', nn.Conv2d(cc, cdim * 2, 5, 1, 2))
        # elif kwargs['sigma_type'] == 'sigma' or type(kwargs['sigma_type']) == float:
        #     self.main.add_module('predict', nn.Conv2d(cc, cdim, 5, 1, 2))
        #     sigma = 0.01
        #     if kwargs['sigma_type'] == 'sigma':
        #         self.register_parameter('logvar', nn.Parameter(2. * torch.log(torch.tensor(sigma, dtype=torch.float))))
        #     else:  # fixed
        #         self.register_buffer('logvar', nn.Parameter(2. * torch.log(torch.tensor(kwargs['sigma_type'], dtype=torch.float))))
    ...
    def forward(self, z, y_cond=None):
        ...
        y = self.main(y)
        # if self.sigma_type != 'z':
        #     y = torch.cat([y, torch.ones_like(y) * self.logvar], dim=1)
        return y
